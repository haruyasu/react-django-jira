...
export const fetchAsyncCreateCategory = createAsyncThunk('task/createCategory', async (item: string) => {
    const res = await axios.post<CATEGORY>(`${process.env.REACT_APP_API_URL}/api/category/`,
        { item: item },
        {
            headers: {
                Authorization: `JWT ${localStorage.localJWT}`
...
export const fetchAsyncCreateTask = createAsyncThunk('task/createTask', async (task: POST_TASK) => {
    const res = await axios.post<READ_TASK>(`${process.env.REACT_APP_API_URL}/api/tasks/`,
        task,
        {
            headers: {
...
export const fetchAsyncUpdateTask = createAsyncThunk('task/updateTask', async (task: POST_TASK) => {
    const res = await axios.put<READ_TASK>(`${process.env.REACT_APP_API_URL}/api/tasks/${task.id}/`,
        task,
        {
            headers: {
                'Content-Type': 'application/json',
